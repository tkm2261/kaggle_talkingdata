...
def data_generator(paths, sc, repeat=True, is_train=True):
    while True:
        for path in paths:
            logger.debug(path)
            ...
            for start in range(0, df.shape[0], batch_size):
                ...
                """
                if is_train:
                    sc.partial_fit(data.values)
                    with open('sc.pkl', 'wb') as f:
                        pickle.dump(sc, f, -1)
                try:
                    _data = sc.transform(data.values)
                    for i, col in enumerate(data):
                        data[col] = _data[:, i]
                except NotFittedError:
                    logger.warn('sc is not fitted. passing')
                    pass
                """
...
def main():
    ...
    train_path = sorted(glob.glob('../data/dmt_train_lstm/*.csv.gz'))
    valid_path = sorted(glob.glob('../data/dmt_valid_lstm/*.csv.gz'))
    ...
    epochs = 10000
    model.fit_generator(generator=data_generator(train_path, sc, is_train=True),
                        steps_per_epoch=steps_per_epoch,
                        epochs=epochs,
                        verbose=2,
                        callbacks=callbacks,
                        validation_data=data_generator(valid_path, sc, is_train=False),
